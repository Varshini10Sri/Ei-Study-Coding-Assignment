export class PlaneStrategy {
    ...
    calculate(options) {
        ...
        let suitability = 50;
        // Cost preference
        if (options.preferences.cost === "low" && cost < 200)
            suitability += 10;
        if (options.preferences.cost === "medium" && cost >= 200 && cost < 500)
            suitability += 15;
        if (options.preferences.cost === "high" && cost >= 500)
            suitability += 20;
        // Time preference
        if (options.preferences.time === "fast" && time < 5)
            suitability += 30;
        if (options.preferences.time === "moderate" && time >= 5 && time < 10)
            suitability += 20;
        if (options.preferences.time === "slow" && time >= 10)
            suitability += 5;
        // Comfort preference
        ...
        if (options.preferences.comfort === "luxury")
            suitability += 25;
        // Distance factor
        if (options.distance > 500)
            suitability += 20;
        if (options.distance < 300)
            suitability -= 15;
