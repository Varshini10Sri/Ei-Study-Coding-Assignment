export class TrainStrategy {
    ...
    calculate(options) {
        ...
        let suitability = 50;
        // Cost preference
        if (options.preferences.cost === "low" && cost < 150)
            suitability += 20;
        if (options.preferences.cost === "medium" && cost >= 150 && cost < 300)
            suitability += 15;
        if (options.preferences.cost === "high" && cost >= 300)
            suitability += 5;
        // Time preference
        if (options.preferences.time === "fast" && time < 4)
            suitability += 10;
        if (options.preferences.time === "moderate" && time >= 4 && time < 8)
            suitability += 20;
        if (options.preferences.time === "slow" && time >= 8)
            suitability += 15;
        // Comfort preference
        ...
        if (options.preferences.comfort === "luxury")
            suitability += 10;
        // Distance factor
        if (options.distance > 200 && options.distance < 800)
            suitability += 15;
