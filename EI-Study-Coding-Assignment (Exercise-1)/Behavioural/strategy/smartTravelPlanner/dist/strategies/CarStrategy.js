export class CarStrategy {
    ...
    calculate(options) {
        ...
        let suitability = 50;
        // Cost preference
        if (options.preferences.cost === "low" && cost < 50)
            suitability += 20;
        if (options.preferences.cost === "medium" && cost >= 50 && cost < 100)
            suitability += 15;
        if (options.preferences.cost === "high" && cost >= 100)
            suitability += 10;
        // Time preference
        if (options.preferences.time === "fast" && time < 3)
            suitability += 20;
        if (options.preferences.time === "moderate" && time >= 3 && time < 6)
            suitability += 15;
        if (options.preferences.time === "slow" && time >= 6)
            suitability += 10;
        // Comfort preference
