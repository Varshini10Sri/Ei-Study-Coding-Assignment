...
export class BusStrategy implements TravelStrategy {
  name = "Bus";
  ...
  calculate(options: TravelOptions) {
    ...
    // Cost preference
    if (options.preferences.cost === "low" && cost < 100) suitability += 25;
    if (options.preferences.cost === "medium" && cost >= 100 && cost < 200)
      suitability += 20;
    if (options.preferences.cost === "high" && cost >= 200) suitability += 5;

    // Time preference
    if (options.preferences.time === "fast" && time < 6) suitability += 10;
    if (options.preferences.time === "moderate" && time >= 6 && time < 12)
      suitability += 20;
    if (options.preferences.time === "slow" && time >= 12) suitability += 15;

    // Comfort preference
    if (options.preferences.comfort === "basic") suitability += 20;
    if (options.preferences.comfort === "standard") suitability += 10;
    if (options.preferences.comfort === "luxury") suitability += 5;

    // Distance factor
    if (options.distance > 100 && options.distance < 600) suitability += 15;
